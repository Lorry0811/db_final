...
\begin{center}
    ...
    \href{https://www.youtube.com/watch?v=eOKiyd8tkkw}{YouTube 影片連結}
\end{center}
...
Relational Database Schema Diagram 中總共有7個table，分別是 \texttt{user} (使用者)、\texttt{posting} (貼文)、\texttt{comment} (留言)、\texttt{class} (貼文類別)、\texttt{course} (課程)、\texttt{favorite\_posts} (收藏貼文) 和 \texttt{report} (舉報)。如下圖 Figure 1 所示。

\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{images/relational_schema.png}
    \caption{Relational Database Schema Design}
...
針對本系統貼文資料量最大的 posting 表，我們以「查詢最新可購買刊登（status='listed' 並依 created\_at DESC 排序，取前 20 筆）」作為代表性場景，實測現有索引（idx\_posting\_status、idx\_posting\_created\_at）相對於無索引時的效能差異。建立索引前（移除相關索引）連續運行五次的結果分別為 3.724 秒、3.711 秒、3.830 秒、3.774 秒、3.777 秒，平均約 3.7632 秒，標準差約 0.0425 秒；保留索引後連續運行五次的結果分別為 0.255 秒、0.207 秒、0.193 秒、0.207 秒、0.222 秒，平均約 0.2168 秒，標準差約 0.0212 秒。實測顯示在大量貼文（約一萬筆）下，索引將平均查詢時間從約 3.76 秒降至 0.22 秒，並降低時間波動，證明索引對此高頻查詢的效能提升顯著。
...
\section{分工資訊}
\begin{itemize}
    \item 資管三 B11705061 羅立宸：Proposal, 系統實作
    \item 資管三 B12705011 黃元翔：Proposal, Report
    \item 資管三 B12705057 陳以倫：Proposal, 錄製影片及上字幕
\end{itemize}

\section{專案心得}
\subsection{資管三 B11705061 羅立宸}
這一次的資料庫期末專案我覺得對於我們是一個不小的考驗，在學期中的時候我們所學的都是純資料庫的知識，我也是第一次嘗試將資料庫透過後端api的方式與前端連接起來，這一次的專案對於我而言真的困難重重，最後還是透過AI的幫助完成了這個任務，但也讓我學習到了很多東西，也讓我對於資料庫的用法了解更加深入，總體而言我還是很開心能夠完成這一次的專案。
\subsection{資管三 B12705011 黃元翔}
這次的資料庫專案的規模應該是進大學以來我遇過最大的，牽涉到的觀念也是最多的，從ERD、正規化，到交易管理、並行控制，甚至最後的Join Algorithm 和NoSQL，這些單獨拿出來都是相當有挑戰性的主題，但在這個專案中我們要把它們融合並實作出來，是一件不簡單的事。一開始的主題發想還算順利，想到主題後就是設計ERD，想data dictionary，交上去之後助教給的回饋讓我發現原來還有很多我沒有想到的部分，補上後整個系統的確更完善了。實作應該是最難的點，多虧組員們的能力十分優秀，讓我在系統實作上並沒有花費太多功夫，可以專心撰寫最後的報告。撰寫報告讓我更理解整個專案的架構、課上教的知識如何在實務上串連起來，每個slide上的規定都有其必要性，這種把所學應用的感覺真好。很感謝授課的教授、給我們建議的助教，以及我的兩位組員，讓我能夠有這樣一個愉快地做專案的經驗。

\subsection{資管三 B12705057 陳以倫}
這次的資料庫專案規模真的非常龐大，從 ER 模型設計、資料庫 Schema 建構、系統 API 撰寫、功能測試到與前端的整合，每一部分都需要投入相當大量的時間與心力。
坦白說，如果沒有 AI 的協助，我真的不可能在期限內完成所有工作。AI 在查詢語法、資料表建構、關聯設定、API 設計、效能優化等方面都幫了非常大的忙。我也深刻體會到，把工具用好、懂得請教正確的問題，能讓整個開發流程事半功倍。
此外，我想特別感謝同組的前端同學。他真的非常強，整個前端界面簡潔美觀功能完善。後來才知道他有修網服這門課，看了他的成果，我下學期很想跟著去修網服精進實力。
